...
object ModelGenerator {
  ...
  def main(args: Array[String]) {
    if (args.length < 1) {
      println("Usage: ModelGenerator <data path> <model persitence path>")
      System.exit(1)
    }

    val spark = SparkSession.builder.appName("Inclass ModelGenerator").getOrCreate()
    ...
    val encoder = Array("NAME_INCOME_TYPE", "NAME_EDUCATION_TYPE", "ORGANIZATION_TYPE")
      .map(column => new OneHotEncoder()
        .setInputCol(column + "_Index")
        .setOutputCol(column + "_Vec"))
